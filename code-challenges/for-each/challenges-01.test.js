...
// why use forEach when it requires two more functions to do the same thing?
const createBetterList = (availableItems) => {
  let myList = [];
  for(let i=0; i<availableItems.length; i++){
    if(availableItems[i].available){
      myList.push(availableItems[i].name);
    }
  }
  return myList;
}
...
const fizzbuzz = (arr) => {
  ...
  arr.forEach(function(element){
    let test = '';
    if(element % fizznum === 0){test = 'Fizz'}
    if(element % buzznum === 0){test += 'Buzz'}
    if(test === 'FizzBuzz'){test = 'Fizz Buzz'}
    if(test != ''){output.push(test)} else {output.push(element)}
  });
  return output;
};
