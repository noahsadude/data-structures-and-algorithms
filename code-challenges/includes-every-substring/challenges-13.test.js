...
const firstLetters = (arr) => {
  let output = [];
  arr.forEach(element => output.push(element[0]));
  return output;
};
...
const standardizePhoneNumbers = (arr) => {
  let output = [];
  arr.forEach(element=>{
    let one = element.substring(1,4);
    let two = element.substring(6,9);
    let three = element.substring(10,14);
...
const allHappy = (arr) => {
  let output = [];
  arr.forEach(element => element.includes(':)') ? output.push(true) : output.push(false));
...
const findEvery = (arr, target) => {
  let output = [];
  arr.forEach(element => element.includes(target) ? output.push(true) : output.push(false));
  for(let i=0;i<output.length;i++){
    if(output[i] === false){
      return false;
    }
