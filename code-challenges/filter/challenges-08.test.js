...
const oddValues = (arr) => arr.filter(element => (element % 2 === 1) ? true : false);
...
const filterStringsWithVowels = (arr) => {
  return arr.filter((element)=>{
    let validate = /[aeiou]/;
    return (validate.test(element)) ? true : false;
  });
};
...
const getStatName = (arr, minBaseStat) => {
  let input = getBaseStatGreaterThan(arr,minBaseStat);
  for(let i = 0; i<input.length; i++){
    input[i] = input[i].stat.name;
  }
  return input;
};
