...
const hasChildrenEntries = (arr, character) => {
  for(let i = 0; i<arr.length; i++){
    let data = Object.entries(arr[i]);
    console.log(data[0][1]);
    if(data[0][1]===character) {
      if(data[2][1][0] === undefined){
        return false;
      }
      return true;
    }
...
const totalCharacters = (arr) => {
  ...
  arr.forEach(element =>{
    ...
    for(let i = 0; i<data[2].length; i++){
      sumCharacters.push(data[2][i]);
    }
    data.pop();
    if(element.spouse !== null){
      sumCharacters.push(element.spouse);
    }
    sumCharacters.push(element.name);
  });
  return sumCharacters.length;
};
