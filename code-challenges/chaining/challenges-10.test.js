...
const count = (target, input) => flatter(input).filter(value => value === target).length;
...
const totalSum = (input) => flatter(input).reduce((total,current) => total += current,0);
...
const divisibleByFiveTwoToThePower = (input) => input.map(arr => arr.filter(val => typeof(val) === 'number' && !(val%5))).map(arr => arr.map(val => Math.pow(2,val)));
